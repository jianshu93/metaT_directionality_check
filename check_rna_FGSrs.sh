...
if [ -d "$database" ]; then
    ...
    for F in $dfiles; do
        ...
        $(./dependencies/FragGeneScanRs -s $output/${SAMPLE}.rename.fasta -p $(nproc) -t complete -w 1 -o $output/${SAMPLE}.FGSrs)
        $(./dependencies/seqtk_linux seq -C $output/${SAMPLE}.FGSrs.ffn > $output/${SAMPLE}.fa)
