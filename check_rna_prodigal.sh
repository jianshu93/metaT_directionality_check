...
if [ -d "$database" ]; then
    ...
    for F in $dfiles; do
        ...
        $(cat $output/${SAMPLE}.rename.fasta >> $output/all_assembly_rename.fasta)
        ./pprodigal -i $output/${SAMPLE}.rename.fasta -f gff -d $output/${SAMPLE}.fna -o $output/${SAMPLE}.gff -p meta -T $(nproc)
...
then
  ...
  database_all_assembly=$output/all_assembly_rename.reformatted
else
  ...
  then
    ...
    ./FastA.reformat.oneline.pl -i $output/all_assembly_rename.fasta -o $output/all_assembly_rename.reformatted
...
#Check to see if the final gene blast file is present
